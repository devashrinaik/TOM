    ...
    def forward(
        ...
        for h in range(min(H, T - 1)):
            ...
            if valid_t > 0:
                ...
                # Pad to full T length so we can stack
                padded = F.pad(err, (0, T - valid_t)) / H
                pattern_errors.append(padded)
        if pattern_errors:
            pattern_error = torch.stack(pattern_errors, dim=0).sum(dim=0)
        else:
...
class ContrastivePartnerMemory(nn.Module):
    ...
    def forward(
        ...
        if types is not None:
            ...
            # Clone memory_bank to avoid in-place modification conflict during enqueue
            memory_sim = torch.mm(queries_flat, self.memory_bank.clone().t()) / self.temperature
...
class UAToM(BaseModel):
    ...
    def __init__(
        ...
        # EMA buffers for stable normalization during inference
        # At eval time (B=1, growing T), batch mean is unstable.
        # EMA tracks running statistics from training for stable normalization.
        self.register_buffer('_sc_ema', torch.ones(1))
        self.register_buffer('_bc_ema', torch.ones(1))
    ...
    def forward(
        ...
        if self.training:
            # Training: use batch mean (stable with large B×T)
            sc_mean = state_changes.mean() + 1e-6
            bc_mean = belief_changes.mean() + 1e-6
            # Update EMA for later use at inference
            self._sc_ema.copy_(0.99 * self._sc_ema + 0.01 * sc_mean.detach())
            self._bc_ema.copy_(0.99 * self._bc_ema + 0.01 * bc_mean.detach())
        else:
            # Inference: use EMA statistics (stable at B=1, small T)
            sc_mean = self._sc_ema
            bc_mean = self._bc_ema
        state_changes_norm = state_changes / sc_mean
        belief_changes_norm = belief_changes / bc_mean
